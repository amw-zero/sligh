...
let compile lexbuf impl_out cert_out =
  ...
  let impl_str = Codegen.string_of_expr evaled_impl [] env in
...
let compile_model_transform input_file transform_script out_file =
  ...
  let _ = List.iter (fun tss -> File.output_tsexpr_list out_file env tss) code in
...
let compile_model input_file out_file = 
  ...
  let model_ast = Parse.parse_with_error lex in
  let env = List.fold_left (fun e s -> Env.add_stmt_to_env s e) Env.empty_env model_ast in
  let compiled = Codegen.string_of_model model_ast env in
